...
//      check user if already exist for given email
    ...
    @Override
    public Boolean userLogIn(String email, String password) throws InvalidUserException {
    ...
    @Override
    public Boolean deleteWalletByEmail(String email) throws WalletException {
        Wallet foundWallet = walletDAO.getWalletByEmail(email);
        if (foundWallet == null) {
            throw new WalletException("Wallet not found for email : " + email);
        }
        else {
            return walletDAO.deleteWalletByEmail(foundWallet);
        }
    }
}
